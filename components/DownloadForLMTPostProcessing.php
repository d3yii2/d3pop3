...
/**
 * Class DownloadForLMTPostProcessing
 * @package d3yii2\d3pop3\components
 *
 * Post processing class which crawls to find a PDF for LMT
 */
class DownloadForLMTPostProcessing implements PostProcessingInterface
{
    ...
    final public function store($url, $modelName, int $modelId): bool
    {
        ...
        $cookieJar = Yii::getAlias('@runtime') . '/cookie';
        ...
        $viewResponse = $this->getResponse($ch);
        ...
        $downloadResponse = $this->getResponse($ch);

        if (!$viewResponse || !$downloadResponse) {
            return false;
        }
