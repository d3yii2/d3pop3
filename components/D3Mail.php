...
class D3Mail
{
            ...
            try {
                ...
                try {
                    ...
                    $err = 'Send exception message: ' . $e->getMessage() . PHP_EOL
                       ...
                       . (
                           isset($transportSymfonymailer)
                            ? 'Can not send email. ' . VarDumper::dumpAsString($transportSymfonymailer)
    ...
    public function saveAddressList(): void
    {
        ...
        foreach ($this->addressList as $address) {
            ...
            $address->email_id = $this->getEmailId();
            if (!$address->save()) {
                if ($address->hasErrors('email_address')) {
                    throw new UserException($address->getFirstError('email_address'));
                }
